...
export async function getAllPromptResults(filters: GetAllPromptResultsFilters): Promise<PromptResult[]> {
	...
	if (clientId && status) query = query.where(and(eq(promptsTable.clientId, clientId), eq(resultsTable.status, status)));
	else if (clientId) query = query.where(eq(promptsTable.clientId, clientId));
	else if (status) query = query.where(eq(resultsTable.status, status));
